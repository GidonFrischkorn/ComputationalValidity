...
get_reliability <- function(data) {
  reliability_pc <- data %>%
    mutate(trialNum = 1:n(), .by = c(ID, Cond)) %>%
    mutate(OddEven = ifelse(trialNum %% 2 == 0, "even","odd")) %>%
    summarize(PC = 1 - mean(Error), .by = c(ID,Cond,OddEven)) %>%
    ...
    pivot_wider(names_from = OddEven,
                values_from = value) %>%
    summarize(reliability = (2*cor(odd,even))/(1 + cor(odd,even)), .by = c(indicator)) %>%
    mutate(measure = "PC")
  ...
  reliability_RT <- data %>%
    ...
    mutate(trialNum = 1:n(), .by = c(ID, Cond)) %>%
    mutate(OddEven = ifelse(trialNum %% 2 == 0, "even","odd")) %>%
    summarize(RT = mean(RT), .by = c(ID,Cond,OddEven)) %>%
    pivot_wider(names_from = Cond,
                values_from = RT) %>%
    mutate(mean = (comp + incomp)/2,
           diff = comp- incomp) %>%
  ...
  reliability_ezDM <- data %>%
    ...
    pivot_wider(names_from = OddEven,
                values_from = value) %>%
    summarize(reliability = (2*cor(odd,even))/(1 + cor(odd,even)), .by = c(indicator, measure))
