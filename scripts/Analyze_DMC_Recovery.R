...
## Load Package Data -----------------------------------------------------------
# Load DMC recovery datasets from package
...
df_behavior_all <- data.table(dmc_recovery_behavior)
df_ezDM_all <- data.table(dmc_recovery_ezDM)
df_reliability_all <- data.table(dmc_recovery_reliability)
...
ggplot(data = avg_effects_behavior %>% filter (indicator %in% c("congruent", "incongruent")),
...
ggplot(data = avg_effects_behavior %>% filter (indicator %in% c("difference")),
...
ggplot(data = df_behavior_all %>% filter(indicator == "difference"),
...
ggplot(data = avg_effects_ezDM %>% filter (indicator %in% c("congruent", "incongruent")),
       aes(x = indicator, y = mean, fill = as.factor(nTrials))) +
  facet_grid(SampleSize ~ measure) +
  geom_boxplot() +
  labs(x = "Condition", y = "Performance", fill = "Trials in each\nCondition") +
...
ggplot(data = avg_effects_ezDM %>% filter(indicator %in% c("difference")),
       aes(x = indicator, y = sd, fill = as.factor(nTrials))) +
  facet_grid(SampleSize ~ measure) +
  geom_boxplot() +
  labs(x = "Condition", y = "Performance", fill = "Trials in each\nCondition") +
